...
cb_ren_final = cb_ren.groupby(["NAME", "Churn"])["Churn"].count().reset_index(name='c_count')
cb_ren_final["percent"] = cb_ren_final.groupby("NAME")["c_count"].apply(lambda x: 100 * x / float(x.sum()))
# fuck me that was hard, need to group em one last time
cb_ren_final = cb_ren_final[cb_ren_final["Churn"] == True]
# merge our data
...
geo_merge.plot(ax=ax, cmap='OrRd', column=geo_merge['percent'], figsize=(12, 12))
